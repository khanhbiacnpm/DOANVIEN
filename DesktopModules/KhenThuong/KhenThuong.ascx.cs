...
{
    public partial class KhenThuong : PortalModuleBase, IActionable
    {        
        ...
        {
            DataTable tb = SqlHelper.ExecuteDataset(strconn, "[HRM_Get_DanhHieuThiDua_Nam1]", nam, loaithanhtich, type).Tables[0];
        ...
        protected void pnlChiTietKhenThuong_Callback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            ...
            DataTable tb = SqlHelper.ExecuteDataset(strconn, "[HRM_KhenThuong_DonVi]", nhom).Tables[0];
            if(tb.Rows.Count>0)
            {
                ...
                LoadDanhHieuThiDua(2, Int32.Parse(row["idthanhtich"].ToString()));
