...
namespace VNPT.Modules.SalaryType
{
    ...
    /// </history>
    /// -----------------------------------------------------------------------------
    partial class ViewBCoefficient : PortalModuleBase, IActionable
    {
        ...
        {                          
            if (!this.IsPostBack)
            {                
                foreach (Philip.Modules.Salary_Group.Salary_GroupInfo p in objGroup.GetParentSalaryGroup(1))
                {
        ...
        {            
                          ...
                          new SqlConnection(str_conn))
            {
               ...
               "SELECT groupname as name,id,parentid FROM Salary_Group WHERE (parentid is null or parentid<=0) and IsActive=1 and type=1 ";
                connection.Open();
        ...
        public DataTable GetParent()
        {
                          ...
                          new SqlConnection(str_conn))
            {
                SqlCommand selectAllCustomers =
                                   connection.CreateCommand();
                selectAllCustomers.CommandText =
               "SELECT groupname as name,id,parentid FROM Salary_Group WHERE type=1 and IsActive=1";
                connection.Open();
                customers.Load(selectAllCustomers.ExecuteReader
                            (CommandBehavior.CloseConnection));
            }
        ...
        public DataTable GetTable()
        {
                          ...
                          new SqlConnection(str_conn))
            {
                SqlCommand selectAllCustomers =
                                   connection.CreateCommand();
                selectAllCustomers.CommandText =
               "SELECT id,groupname as name FROM Salary_Group where type=1 and isactive=1  ORDER BY name DESC ";                
                connection.Open();
                customers.Load(selectAllCustomers.ExecuteReader
                            (CommandBehavior.CloseConnection));
            }
