...
{
    public partial class PrintSoYeuLyLich1 : PortalModuleBase, IActionable
    {
                        ...
                        foreach (TreeListNode p in nodeParent.ChildNodes)
                        {
                            if (p.Key.Contains("u"))
                            {
                                ...
                                if (Convert.ToDecimal(p.Key.Substring(1)) == UnitChildId)
                                {
                                    ...
                                    {
                                        if (Convert.ToDecimal(idEmp) == Convert.ToInt32(c.Key.Substring(1)))
                                        {
                                            c.Expanded = true;
                                            c.Focus();
                                        }
        ...
        protected void callbackPanel_Callback(object source, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            ...
            //this.soluoclilich.Attributes["src"] = null;
            if (id.Contains("e"))
            {
                int ItemId = Int32.Parse(id.Substring(1));
               // this.employees = objEmployees.GetEmployees(ItemId);
                this.soluoclilich.Attributes["src"] = DotNetNuke.Common.Globals.ApplicationPath + "/Default.aspx?tabid=329&Id=" + ItemId;
            }
        }
