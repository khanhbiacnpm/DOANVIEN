...
namespace VNPT.Modules.DaoTao
{
    ...
    {        
        ...
        {
            DataTable tb = SqlHelper.ExecuteDataset(strconn, "[HRM_GET_KHOADAOTAO_KEHOACHDAIHAN_IDCN]", str, 0).Tables[0];
            cmb_khoadaotao.DataSource = tb;
            cmb_khoadaotao.TextField = "tenkhoadaotao";
            cmb_khoadaotao.ValueField = "id";
            cmb_khoadaotao.DataBind();
            //cmb_khoadaotao.Items.Insert(0, new ListEditItem("-- Chọn --", "0"));
            //cmb_khoadaotao.SelectedIndex = 0;
            if (tb.Rows.Count == 0)
        ...
        protected void gridKhoaDaoTao_OnCustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            load_data_grid(e.Parameters);
        }
        private void load_data_grid(object nam)
        {
            DataTable tb = SqlHelper.ExecuteDataset(strconn, "HRM_GET_KHOADAOTAO", nam,1, 0).Tables[0];
            gridKhoaDaoTao.DataSource = tb;
            gridKhoaDaoTao.DataBind();
        }
        private void load_comb_nam()
        {
            DataSet ds = SqlHelper.ExecuteDataset(strconn, "[HRM_GET_KHOADAOTAO]", 0,1, 0);
        ...
        {            
            ...
            cmb_chuyennganh.DataSource = ds.Tables[7];
            ...
            if (item != null)
                ...
                DataTable tb = SqlHelper.ExecuteDataset(strconn, "[HRM_GET_KHOADAOTAO_NHANVIEN]", gridKhoaDaoTao.GetRowValues(gridKhoaDaoTao.EditingRowVisibleIndex,"id"), 0).Tables[0];
        ...
        protected void cb_luukhoadaotao_OnCallback(object sender, DevExpress.Web.ASPxClasses.CallbackEventArgsBase e)
        {
            ...
            if (e.Parameter.Contains("save"))
            {
                ...
                if (check_capnhat.Checked)
                {
                    ...
                    SqlHelper.ExecuteNonQuery(strconn, "HRM_KHOADAOTAODAIHAN_UI", cmb_khoadaotao.Value, 0, tenkhoadaotao, "", 0, 0, null, null, "", 2);
                }
                object idkhoadaotao = SqlHelper.ExecuteScalar(strconn, "[HRM_KHOADAOTAO_UI]", 0, txt_madaotao.Text, idkhoadaotaokehoach, tenkhoadaotao, "",
                    ...
                    cmb_trinhdo.Value, cmb_chuyennganh.Value, cmb_hinhthucdaotao.Value,
                    date_tungay.Value, date_denngay.Value, filename, memo_ghichu.Text, cmb_diadiemtochuc.Value, txt_donvitochuc.Text, check_camket.Checked, 1);
            ...
            else if (e.Parameter.Contains("update"))
            {
                ...
                cmb_trinhdo.Value == null ? 0 : cmb_trinhdo.Value, cmb_chuyennganh.Value == null ? 0 : cmb_chuyennganh.Value,
                cmb_hinhthucdaotao.Value == null ? 0 : cmb_hinhthucdaotao.Value,
            ...
            {
                SqlHelper.ExecuteNonQuery(strconn, "[HRM_GET_KHOADAOTAO]", e.Parameter.Replace("delete",""),1, 10);
            ...
            else if (e.Parameter.Contains("loadedit"))
            {
                ...
                if (tb1.Rows.Count > 0)
                {
                    ...
                    var itemkhoadaotao = cmb_khoadaotao.Items.FindByValue(tb1.Rows[0]["idtrinhdo"]);
